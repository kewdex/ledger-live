...
import { bypassCoinXrpRecipientCache, getBridges } from "../helpers";
...
function makeTransactions(): XrpScenarioTransaction[] {
  ...
  // Sub-reserve transfer to the now-existing recipient. Works because
  // `bypassCoinXrpRecipientCache()` forces a fresh `account_info` query on each check,
  // so the recipient is no longer treated as new after tx 1 is validated.
...
export const scenarioXrp: Scenario<GenericTransaction, Account> = {
  ...
  setup: async () => {
    ...
    bypassCoinXrpRecipientCache();

    await spawnRippled();

    const { signer, address } = buildXrpSigner();
    ...
    const { currencyBridge, accountBridge } = await getBridges(signer);
