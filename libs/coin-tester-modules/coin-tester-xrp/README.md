...
`coin-xrp` keeps a 10 s in-memory recipient cache that, after tx 1's
creation, still reports the recipient as new — and would reject tx 2's
sub-reserve send. `helpers.ts::bypassCoinXrpRecipientCache` reassigns
`cachedRecipientIsNew` to a non-caching passthrough at setup, so every
pre-flight re-queries `account_info` directly. Real LLD/LLM users never
trip the cache (UI pacing > TTL), so this patch is test-only.
