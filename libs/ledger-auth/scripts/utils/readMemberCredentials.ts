...
export async function readMemberCredentials(): Promise<MemberCredentials> {
  ...
  if (!validateMemberCredentials(credentials)) {
    throw new Error("Invalid credentials");
  }
  return credentials;
}

function validateMemberCredentials(credentials: unknown): credentials is MemberCredentials {
  ...
  if (
    ...
    throw new Error('Credentials must contain string "trustchainId", "pubkey" and "privatekey"');
  }
  if (pubkey.length !== 66) {
    // LKRP public keys have a 1 byte prefix, so 33 bytes = 66 hex chars.
    throw new Error(`Expected pubkey to be 66 hex chars, got ${pubkey.length}`);
  }
  if (privatekey.length !== 64) {
    throw new Error(`Expected privatekey to be 64 hex chars, got ${privatekey.length}`);
  }
