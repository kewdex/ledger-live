...
const mockGetProgramTokenBalance = jest.mocked(apiClient.getProgramTokenBalance);
...
function getMockedTokenOperation(overrides?: Partial<AleoOperation>): AleoOperation {
  return getMockedOperation({
    ...
    extra: {
      ...
      tokenInfo: { programId: MOCK_TOKEN_PROGRAM_ID },
    },
    ...overrides,
  });
}
...
describe("tokens.ts", () => {
  beforeEach(() => {
    ...
    mockGetProgramTokenBalance.mockResolvedValue("250000u128");
  });
  ...
  describe("prepareTokenOperations", () => {
    ...
    it("should skip token operations without tokenInfo or CAL match", async () => {
      const tokenOp = getMockedTokenOperation({
  ...
  describe("getAleoSubAccounts", () => {
    it("should fetch transparent balances and build token sub-accounts", async () => {
      ...
      expect(mockGetProgramTokenBalance).toHaveBeenCalledWith(
        mockCurrency,
        MOCK_TOKEN_PROGRAM_ID,
        address,
      );
    ...
    it("should return an empty array when there are no token operations", async () => {
      ...
      expect(result).toEqual([]);
      expect(mockGetProgramTokenBalance).not.toHaveBeenCalled();
    });
  });
